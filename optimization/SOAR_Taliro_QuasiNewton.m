function [run, history] = SOAR_Taliro_QuasiNewton(inpRanges,opt)
...
while(sim_count < nSamples)
   ...
   fun = @(x)Compute_Robustness(x);
   [x1,fval,exitflag,output] = fminunc(fun,x0,options)
   ...
   a = min(inpRanges(:,2)-inpRanges(:,1)); %THIS PARAMETER MUST BE TUNED!
   gk = ones(nInputs,1);
